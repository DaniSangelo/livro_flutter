...
class PalavraDAL {
  Future<String> insert({@required PalavraModel palavraModel}) async {
    String result;
    try {
      var localPersistence = SQFLiteDataBase.instance;

      var uuid = Uuid();
      palavraModel.palavraId = uuid.v1();
      result = palavraModel.palavraId;

      var recordsAffected = await localPersistence.database
          .insert(kClienteTableName, _mapModelToJSon(clienteModel));
      if (recordsAffected == 0) result = null;
    } catch (exception, stackTrace) {
      await ErrorsApp.showError(
          'CLIENTE_DAL->insert()', exception.toString(), stackTrace.toString());
      return null;
    }
    return result;
  }
